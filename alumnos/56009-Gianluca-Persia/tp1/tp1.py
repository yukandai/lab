...
def queuewords(queue, finalqueue):
    while queue.qsize() != 0: #si la cola tiene algun contenido
        ...
        words = count(read) #llamo a count
...
def help(): #ayuda
    ...
    mesagge = "-n [bytes de tamano]"
    print mesagge
    print "Puede usar lo siguiente ./tp1.py -f prueba.txt -p 2 -n 2048"
    exit(0) #salir
...
processlist = childprocess() #invoco funcion

open(value, entry, filepath) #invoco funcion
